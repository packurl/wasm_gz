const url=new URL('gz.wasm',import.meta.url);
await (await fetch(url)).arrayBuffer();
const worker=await new Promise(r=>{
  // For browsers that don't support type: module on workers (firefox < 114, safari < 15)
  // const worker=new Worker(new URL('./gz_worker_script.js',import.meta.url));
  const worker=new Worker(new URL('./gz_worker_script.mjs',import.meta.url),{type:'module'});
